...
/***************** SELECT DIFFICULTY **********************/
$select = $pdo->query("SELECT a.id id_article, d.name difficulty_name
                                 FROM difficulty d
...
/***************** SELECT AUTEUR ********************************/
$select = $pdo->query("SELECT a.id id_article, u.username username_name
                                 FROM user u
                                 INNER JOIN article a
                                 ON a.authorId = u.id
                                 WHERE a.id = $articleId");
...
/***************** SELECT COMMENTS *****************************/
$userId = $_SESSION['auth']->id;
$select = $pdo->query("SELECT * FROM comment WHERE userId = $userId AND articleId = $articleId");
$select->setFetchMode(PDO::FETCH_ASSOC);
